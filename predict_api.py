...
try:
    with open(model_path, 'rb') as model_file:
        logger.debug(f"File size: {os.path.getsize(model_path)} bytes")
        try:
...
def predict():
    try:
        ...
        logger.debug(f"Raw Inputs - Brand: {brand}, Model: {model_name}, Transmission: {transmission}, Owner: {owner}, Fuel Type: {fuel_type}")
